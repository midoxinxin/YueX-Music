...
//yy,添加了随机播放,去掉了playcounts\repeat变量,存在的问题是歌曲播放结束后下一首是哪一个？
	...
	var time = new Date(),
		...
		trigger = false,
		audio, timeout, isPlaying,currentTrack;


	//yy随机播放控制
	// var currentTrack= Math.floor(Math.random()*10+0);
	...
	var switchTrack = function(i){
		...
		$('audio').remove();
		loadMusic(track);
		loadComment(track); //珺改，载入当前的评论页面
	...
	//yy 顺序播放可实现
		if (nowPlay >= playlist.length-1){
			switchTrack(0);
		}else if (nowPlay < playlist.length-1){
			switchTrack(++nowPlay);
		}
	}
...
/*********************** 自适应END*****************************/
	...
	//yy add	
	if(localStorage.getItem("my_collectList")){
		var str = localStorage.getItem("my_collectList");//获取之前存储的数据
	}
	else{
		var str = my_collectList;
	...
	var loadMusic = function(i){
		...
		//yy点击后，判断collectList数组中是否有该项，有代表要移除，无则代表要添加
		collector.onclick = function(event){
				for(k=0; k<=my_collectList.length;k++){

				//如果当前播放序列为i，判断该序列是否在缓存数组内
				//在，移除该项
	...
	$('#playlist li').each(function(i){
		...
		$(this).on('click', function(){
			...
			//yy收藏图标的变化	
			for (var y=0; y<my_collectList.length; y++){
				if(my_collectList[y] == _i ){
					$('#collector').removeClass('glyphicon-heart-empty').addClass('glyphicon-heart');
					break;
				}
				else{
